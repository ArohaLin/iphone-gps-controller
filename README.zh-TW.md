...
- **步數計數（HealthKit）** — 可增加模擬步數與「模擬行走」模式；步數透過 iOS 捷徑寫入 iPhone **健康** App（見 [步數計數](#-步數計數)）。
...
| 👟 步數計數 | 增加步數 / 模擬行走，透過 iOS 捷徑寫入 iPhone 健康 |
...
| **網路** | 後端預設僅限本機；搜尋 / 時區需連網。寫入步數需額外工具 — 見 [步數計數](#-步數計數) |
...
| `HOST` | 綁定位址。設 `0.0.0.0` 可讓 iPhone 透過 Wi-Fi 連到 API（步數輪詢備援需要） | `127.0.0.1` |

範例：

```bash
sudo python3 gps_launcher.py 9000              # 自訂埠
sudo python3 gps_launcher.py 8090 0.0.0.0      # 開放區網（Wi-Fi 步數輪詢）
```
...
| `SHORTCUT_NAME` | 用來把步數寫入健康的 iOS 捷徑名稱 | `GPS步數增加` |

### 日誌

每次執行會寫入 `gps_launcher.log`（與腳本同名、副檔名 `.log`），同時輸出到主控台。日誌**不應上傳** — 內含裝置名稱、UDID 與本次 session token。
...
當連接兩支以上 iPhone 時，裝置清單頂端會出現 **🔗 同步全部裝置** 卡片。選取它後，所有動作（設定位置、清除、巡航步進、步數）都會**同步廣播到所有已連線裝置**。卡片會顯示連線數（例如 `2 / 2`）。
...
**步數** 分頁可增加模擬步數，並執行「模擬行走」（每隔幾秒增加 N 步、持續一段總時間）。在同步模式下步數也可廣播到所有裝置。

### 步數如何進到 iPhone 健康 App

GPS 模擬走的是開發者（DVT）通道，手機端不需任何 App。**但 HealthKit 步數不同** — 必須由具有健康寫入權限的 App/捷徑寫入。本工具透過驅動一個 **iOS 捷徑** 把步數記錄到健康。

**iPhone 端一次性設定：**
1. 開啟 **捷徑** App → 新增捷徑。
2. 名稱需與 `SHORTCUT_NAME` 完全一致（預設 `GPS步數增加`）。
3. 加入動作 **記錄健康數據** → 類型：**步數** → 數量：**捷徑輸入**。
4. 在捷徑設定中開啟 **不需確認即可執行**。

**Launcher 觸發方式（自動偵測，依序）：**

| 方式 | iOS | 需求 |
|------|-----|------|
| `xcrun devicectl` | iOS 17+ | 完整版 **Xcode 15+**（非僅命令列工具） |
| `ideviceopenurl` | iOS 12–16 | `brew install libimobiledevice` |
| Wi-Fi 輪詢（備援） | 任意 | 以 `HOST=0.0.0.0` 啟動；另設一個輪詢捷徑從 Launcher 取得待寫入步數 |

步數分頁內含 **🔗 iPhone 捷徑設定** 面板，會顯示目前生效的方式與備援用的輪詢 URL。

> 若以上皆不可用，步數會放入待處理佇列，但在設定觸發方式前不會出現在健康 App。
...
| `GET /device/{idx}/steps` | 目前步數 |
| `POST /device/{idx}/steps/add` | 增加步數 — body `{ "count": N }` |
| `POST /device/{idx}/steps/remove` | 減少步數 — body `{ "count": N }` |
| `POST /device/{idx}/steps/reset` | 步數歸零 |
...
| `POST /devices/steps/add` | 對每台裝置增加步數 |
| `POST /devices/steps/remove` | 對每台裝置減少步數 |

### 捷徑整合

| 方法與路徑 | 說明 |
|-----------|------|
| `GET /shortcut/info` | 整合狀態（生效方式、token、本機 IP、待處理步數） |
| `POST /shortcut/config` | 更新捷徑名稱 — body `{ "name": ".." }` |
| `GET /shortcut/sync` | iPhone 捷徑輪詢端點（token 驗證） |
| `POST /shortcut/ack` | 確認健康寫入完成（token 驗證） |
...
**Q6：步數加了但健康 App 沒有任何記錄？**
- 步數需要 iOS 捷徑觸發 — 見 [步數計數](#-步數計數)
- 可在步數分頁的 **🔗 iPhone 捷徑設定** 面板查看目前生效的方式
